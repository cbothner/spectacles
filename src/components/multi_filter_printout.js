...
const MultiFilterPrintout = ({suggestions, filtersById}) => {
  return <div style={{margin: '1.5em 2em'}}>
    {suggestions.map( suggestion => {
      ...
      return <div className="pt-card" style={{marginBottom: '1em'}}>
        <div style={{display: 'flex', justifyContent: 'space-between'}}>
          <div>
            <p>
              <strong style={{fontSize: 20, marginRight: '0.5em'}}>{filter.name}</strong>
              <span className="pt-text-muted" style={suggestion.specialPrice &&  {textDecoration: 'line-through'}}>
                ${filter.basePrice} per pair
              </span>
              {" "}
              {suggestion.specialPrice && <span style={{color: Colors.GREEN1}}>
                Your price: ${suggestion.specialPrice}</span>}
            </p>
          </div>
